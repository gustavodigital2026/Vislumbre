...
const LineChart = ({ dados }) => {
    ...
    const pathD = points.length > 1 ? points.map((p, i) => (i === 0 ? `M ${p.x} ${p.y}` : `L ${p.x} ${p.y}`)).join(" ") : "";
    return (
...
const StatsPanel = ({ pedidos, servicos, voltar }) => {
  ...
  const calcularMetricas = () => {
    const stats = {}; const financeiro = {}; const vendasRaw = {}; let faturamentoTotal = 0;
    servicos.forEach(s => financeiro[s.nome] = 0); financeiro["Outros"] = 0;
    if (viewMode === 'dia') { let curr = new Date(dataInicio + "T00:00:00"); const end = new Date(dataFim + "T23:59:59"); if (curr < end && (end - curr) < 31536000000) { while (curr <= end) { vendasRaw[curr.getTime()] = 0; curr.setDate(curr.getDate() + 1); } } }
    const inicioTs = new Date(dataInicio + "T00:00:00").getTime(); const fimTs = new Date(dataFim + "T23:59:59.999").getTime();
    ...
    const aggregated = {}; const getWeekNumber = (d) => { d = new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate())); d.setUTCDate(d.getUTCDate() + 4 - (d.getUTCDay()||7)); var yearStart = new Date(Date.UTC(d.getUTCFullYear(),0,1)); return Math.ceil(( ( (d - yearStart) / 86400000) + 1)/7); }
    Object.keys(vendasRaw).forEach(ts => {
        const date = new Date(Number(ts)); const valor = vendasRaw[ts]; let key = "", label = "", sortVal = Number(ts);
        if (viewMode === "dia") { key = ts; label = `${date.getDate().toString().padStart(2,'0')}/${(date.getMonth()+1).toString().padStart(2,'0')}`; } 
        else if (viewMode === "semana") { const week = getWeekNumber(date); key = `${date.getFullYear()}-W${week}`; label = `Sem ${week}`; sortVal = week; } 
        else if (viewMode === "mes") { key = `${date.getFullYear()}-${date.getMonth()}`; const meses = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"]; label = `${meses[date.getMonth()]}/${date.getFullYear().toString().substr(2)}`; sortVal = date.getFullYear()*100 + date.getMonth(); } 
        else if (viewMode === "ano") { key = `${date.getFullYear()}`; label = key; sortVal = date.getFullYear(); }
  ...
  const gerarGraficoPizza = () => { if (faturamentoTotal === 0) return "conic-gradient(#ccc 0% 100%)"; let acumulado = 0; const segmentos = Object.keys(financeiro).map(key => { const valor = financeiro[key]; if (valor === 0) return null; const servicoObj = servicos.find(s => s.nome === key); const cor = servicoObj ? servicoObj.cor : "#95a5a6"; const porcentagem = (valor / faturamentoTotal) * 100; const inicio = acumulado; acumulado += porcentagem; return `${cor} ${inicio}% ${acumulado}%`; }).filter(Boolean); return `conic-gradient(${segmentos.join(", ")})`; };

  return (
    <div style={{padding: "30px", background: "#f4f6f8", height: "100vh", overflowY: "auto", boxSizing: "border-box"}}>
        <div style={{maxWidth: "1100px", margin: "0 auto"}}>
            <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px", background: "white", padding: "15px", borderRadius: "8px", boxShadow: "0 2px 5px rgba(0,0,0,0.05)"}}>
                <div><h2 style={{margin:0, color: "#2c3e50"}}>📊 Estatísticas</h2></div>
                <div style={{display: "flex", gap: "10px", alignItems: "center"}}>
                    <label style={{fontSize: "12px", fontWeight: "bold"}}>De:</label><input type="date" value={dataInicio} onChange={e=>setDataInicio(e.target.value)} style={{padding: "5px", border: "1px solid #ccc", borderRadius: "4px"}} />
                    <label style={{fontSize: "12px", fontWeight: "bold"}}>Até:</label><input type="date" value={dataFim} onChange={e=>setDataFim(e.target.value)} style={{padding: "5px", border: "1px solid #ccc", borderRadius: "4px"}} />
                    <button onClick={voltar} style={{padding: "8px 15px", background: "#95a5a6", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", marginLeft: "10px"}}>Sair</button>
                </div>
            </div>
            <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "30px"}}>
                <div style={{background: "white", padding: "20px", borderRadius: "8px", boxShadow: "0 2px 5px rgba(0,0,0,0.05)"}}>
                    <h3 style={{marginTop: 0, color: "#27ae60", borderBottom: "1px solid #eee", paddingBottom: "10px"}}>💰 Faturamento</h3>
                    {Object.keys(financeiro).map(k => (<div key={k} style={{display: "flex", justifyContent: "space-between", marginBottom: "10px"}}><span>{k}</span><strong>{formatarMoeda(financeiro[k])}</strong></div>))}
                    <div style={{display: "flex", justifyContent: "space-between", marginTop: "15px", paddingTop: "10px", borderTop: "2px dashed #ddd", fontSize: "18px"}}><strong>TOTAL</strong><strong style={{color: "#27ae60"}}>{formatarMoeda(faturamentoTotal)}</strong></div>
                </div>
                <div style={{background: "white", padding: "20px", borderRadius: "8px", boxShadow: "0 2px 5px rgba(0,0,0,0.05)", display: "flex", alignItems: "center", justifyContent: "space-around"}}>
                    <div style={{width: "150px", height: "150px", borderRadius: "50%", background: gerarGraficoPizza(), border: "4px solid white", boxShadow: "0 0 10px rgba(0,0,0,0.1)"}}></div>
                    <div style={{fontSize: "13px"}}>{servicos.map(s => (<div key={s.id} style={{marginBottom: "5px"}}><span style={{display:"inline-block", width:"10px", height:"10px", background:s.cor, marginRight:"5px"}}></span>{s.nome}</div>))}</div>
                </div>
            </div>
            <div style={{background: "white", borderRadius: "8px", padding: "20px", marginBottom: "30px", boxShadow: "0 2px 5px rgba(0,0,0,0.05)"}}>
                <div style={{display: "flex", justifyContent: "space-between", alignItems: "center"}}>
                    <h3 style={{marginTop: 0, color: "#2c3e50"}}>📈 Evolução</h3>
                    <div style={{display:"flex", gap:"5px"}}>{['dia', 'semana', 'mes', 'ano'].map(m => (<button key={m} onClick={()=>setViewMode(m)} style={{padding:"5px 10px", border:"1px solid #2980b9", borderRadius:"4px", cursor:"pointer", background: viewMode === m ? "#2980b9" : "white", color: viewMode === m ? "white" : "#2980b9", textTransform: "capitalize", fontSize: "12px", fontWeight: "bold"}}>{m === 'mes' ? 'Mês' : m}</button>))}</div>
                </div>
...
const AdminTeamPanel = ({ voltar }) => {
  ...
  useEffect(() => { const unsub = onSnapshot(query(collection(db, "usuarios")), (snap) => { setUsuarios(snap.docs.map(d => ({ id: d.id, ...d.data() }))); }); return () => unsub(); }, []);
  const adicionarUsuario = async () => { if (!novoNome || !novoLogin || !novaSenha) return alert("Preencha tudo!"); await addDoc(collection(db, "usuarios"), { nome: novoNome, login: novoLogin, senha: novaSenha, role: "operador", acessoStats: false }); setNovoNome(""); setNovoLogin(""); setNovaSenha(""); alert("Usuário criado!"); };
  const removerUsuario = async (id) => { if (window.confirm("Remover?")) await deleteDoc(doc(db, "usuarios", id)); };
  const toggleStats = async (id, statusAtual) => { await updateDoc(doc(db, "usuarios", id), { acessoStats: !statusAtual }); };
  return (
    <div style={{padding: "40px", background: "#ecf0f1", height: "100vh", overflowY: "auto"}}><div style={{maxWidth: "600px", margin: "0 auto"}}><div style={{display:"flex", justifyContent:"space-between", marginBottom:"20px"}}><h2>Gestão de Equipe</h2><button onClick={voltar}>Voltar</button></div><div style={{background: "white", padding: "20px", borderRadius: "8px", marginBottom: "20px"}}><h4>Novo Usuário</h4><input placeholder="Nome" value={novoNome} onChange={e=>setNovoNome(e.target.value)} style={{display:"block", width:"100%", padding:"8px", marginBottom:"10px"}} /><input placeholder="Login" value={novoLogin} onChange={e=>setNovoLogin(e.target.value)} style={{display:"block", width:"100%", padding:"8px", marginBottom:"10px"}} /><input placeholder="Senha" value={novaSenha} onChange={e=>setNovaSenha(e.target.value)} style={{display:"block", width:"100%", padding:"8px", marginBottom:"10px"}} /><button onClick={adicionarUsuario} style={{width: "100%", padding: "10px", background: "#2980b9", color: "white", border: "none", cursor: "pointer"}}>Cadastrar</button></div>
    {usuarios.map(u => (
        <div key={u.id} style={{background: "white", padding: "10px", marginBottom: "5px", display: "flex", justifyContent: "space-between", alignItems: "center"}}>
            <div><strong>{u.nome}</strong> ({u.login})</div>
            <div style={{display:"flex", gap:"10px", alignItems:"center"}}>
                <label style={{fontSize:"11px", display:"flex", alignItems:"center", gap:"5px", cursor:"pointer"}}><input type="checkbox" checked={u.acessoStats || false} onChange={() => toggleStats(u.id, u.acessoStats)} /> Ver Stats</label>
                <button onClick={() => removerUsuario(u.id)} style={{color: "red", border: "none", background: "none", cursor: "pointer"}}>X</button>
            </div>
        </div>
    ))}</div></div>
  );
...
const AdminServicesPanel = ({ servicos, voltar }) => {
    ...
    const adicionar = async () => { if (!nome) return alert("Digite o nome"); await addDoc(collection(db, "servicos"), { nome, cor }); setNome(""); };
    const remover = async (id) => { if(window.confirm("Remover?")) await deleteDoc(doc(db, "servicos", id)); };
    return (
        <div style={{padding: "40px", background: "#ecf0f1", height: "100vh", overflowY: "auto"}}><div style={{maxWidth: "600px", margin: "0 auto"}}><div style={{display:"flex", justifyContent:"space-between", marginBottom:"20px"}}><h2>🛠️ Gerenciar Serviços</h2><button onClick={voltar}>Voltar</button></div><div style={{background: "white", padding: "20px", borderRadius: "8px", marginBottom: "20px"}}><h4>Adicionar Novo Serviço</h4><div style={{display:"flex", gap:"10px"}}><input placeholder="Nome (ex: Social Media)" value={nome} onChange={e=>setNome(e.target.value)} style={{flex:1, padding:"10px"}} /><input type="color" value={cor} onChange={e=>setCor(e.target.value)} style={{height:"40px", width:"50px"}} /><button onClick={adicionar} style={{background:"#27ae60", color:"white", border:"none", padding:"0 15px", borderRadius:"4px"}}>Salvar</button></div></div>{servicos.map(s => (<div key={s.id} style={{background: "white", padding: "10px", marginBottom: "5px", display: "flex", justifyContent: "space-between", alignItems: "center", borderLeft: `5px solid ${s.cor}`}}><strong>{s.nome}</strong><button onClick={() => remover(s.id)} style={{color: "red", border: "none", background: "none", cursor: "pointer"}}>X</button></div>))}</div></div>
    );
};
...
export default function App() {
  ...
  useEffect(() => { const unsub = onSnapshot(collection(db, "servicos"), (snap) => { const lista = snap.docs.map(d => ({ id: d.id, ...d.data() })); if(lista.length===0){ addDoc(collection(db, "servicos"), { nome: "Jingle", cor: "#e67e22" }); addDoc(collection(db, "servicos"), { nome: "Vídeo", cor: "#3498db" }); } else setServicos(lista); }); return () => unsub(); }, []);
  useEffect(() => { localStorage.setItem("vislumbre_google_key", apiKey); }, [apiKey]);
  useEffect(() => { localStorage.setItem("vislumbre_model", modeloIA); }, [modeloIA]);
  ...
  const adicionarLead = async () => {
    if (!novoTel) return;
    try { await addDoc(collection(db, "pedidos"), { cliente: "", telefone: novoTel, status: "leads", obs: "", servico: servicos[0]?.nome || "Outros", valorRaw: "", comprovanteUrl: null, audios: [], roteiro: "", tsEntrada: Date.now(), dataEntrada: new Date().toLocaleString(), historicoAcoes: [{ user: currentUser.nome, desc: "Criou o Lead", data: new Date().toLocaleString(), timestamp: Date.now() }] }); setNovoTel(""); } catch (e) { alert("Erro: " + e.message); }
  };
  ...
  const moverPara = async (id, novoStatus) => {
    ...
    if (novoStatus === "finalizados") { 
        ...
        if(pedido.audios && pedido.audios.length > 0) {
            await Promise.all(pedido.audios.map(async (url) => { try { const fileRef = ref(storage, url); await deleteObject(fileRef); } catch(e) { console.error("Erro deletar:", e); } }));
            updates.audios = [];
        }
    }
  ...
  const handleAudioUpload = async (e, id) => {
    ...
    if (file) {
       const btn = e.target.nextSibling; if(btn) btn.innerText = "⏳ Enviando...";
       try {
           const storageRef = ref(storage, `audios/${Date.now()}_${file.name}`);
           await uploadBytes(storageRef, file);
           const url = await getDownloadURL(storageRef);
           const pedido = pedidos.find(p => p.id === id);
           await updateDoc(doc(db, "pedidos", id), { audios: [...(pedido.audios || []), url], historicoAcoes: getNovoHistorico(pedido, "Anexou Áudio (Nuvem)") });
       } catch (err) { alert("Erro no upload: " + err.message); } finally { if(btn) btn.innerText = "← Adicionar áudio"; }
    }
    e.target.value = null;
  };
  ...
  const handleResetSystem = async () => {
      ...
      if (!confirm1) return;
      const confirm2 = window.confirm("Última chance: Essa ação é irreversível.\n\nClique em OK para apagar tudo.");
      if (confirm2) {
          const snap = await getDocs(collection(db, "pedidos"));
          const promises = snap.docs.map(d => deleteDoc(doc(db, "pedidos", d.id)));
  ...
  const HistoricoView = ({ historico }) => (
      <div style={{marginTop: "30px", borderTop: "2px solid #ecf0f1", paddingTop: "20px"}}>
          <h4 style={{margin: "0 0 10px 0", color: "#7f8c8d"}}>📜 Histórico de Atividades</h4>
          <div style={{background: "#f9f9f9", padding: "10px", borderRadius: "6px", maxHeight: "150px", overflowY: "auto", border: "1px solid #eee"}}>
              {(!historico || historico.length === 0) && <span style={{fontSize:"12px", color:"#ccc"}}>Nenhuma atividade registrada.</span>}
              {historico && historico.map((h, i) => (
                  <div key={i} style={{fontSize: "12px", marginBottom: "8px", borderBottom: "1px dashed #ddd", paddingBottom: "4px"}}>
                      <span style={{fontWeight: "bold", color: "#2c3e50"}}>{h.user}</span> <span style={{color: "#7f8c8d"}}> - {h.data}</span>
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", fontFamily: "Segoe UI, sans-serif" }} onClick={() => setShowConfig(false)}>
      <header style={{ padding: "15px", background: "#2c3e50", color: "white", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{display:"flex", gap:"10px", alignItems:"center"}}>
            ...
            <span style={{fontSize: "12px", background: "#f1c40f", color: "black", padding: "2px 8px", borderRadius: "10px", fontWeight:"bold"}}>{currentUser.nome} ({currentUser.role === "admin" ? "Admin" : "Op"})</span>
            
            {currentUser.role === "admin" && (
                <div style={{position: "relative"}}>
                    <button onClick={(e) => { e.stopPropagation(); setShowConfig(!showConfig); }} style={{background:"none", border:"none", cursor:"pointer", fontSize:"22px", marginLeft:"15px"}}>⚙️</button>
                    {showConfig && (
                        <div style={{
                            position: "absolute", top: "35px", left: "0", background: "white", color: "#333", 
                            boxShadow: "0 4px 10px rgba(0,0,0,0.2)", borderRadius: "6px", overflow: "hidden", zIndex: 100, width: "150px"
                        }}>
                            <div onClick={() => setAba("admin_team")} style={{padding: "10px", cursor: "pointer", borderBottom: "1px solid #eee", fontSize: "14px"}}>👥 Equipe</div>
                            <div onClick={() => setAba("admin_services")} style={{padding: "10px", cursor: "pointer", borderBottom: "1px solid #eee", fontSize: "14px"}}>🛠️ Serviços</div>
                            <div onClick={() => setAba("stats")} style={{padding: "10px", cursor: "pointer", borderBottom: "1px solid #eee", fontSize: "14px"}}>📊 Stats</div>
                            <div style={{padding: "10px", borderBottom: "1px solid #eee"}}>
                                <input value={apiKey} onClick={e=>e.stopPropagation()} onChange={e => setApiKey(e.target.value)} placeholder="API Key..." type="password" style={{width: "100%", border:"1px solid #ddd", padding:"4px", fontSize:"12px"}} />
                            </div>
                            <div onClick={handleResetSystem} style={{padding: "10px", cursor: "pointer", color: "red", fontWeight: "bold", fontSize: "14px", background: "#fff5f5"}}>🗑️ Resetar Tudo</div>
                        </div>
                    )}
                </div>
            )}
            ...
            {currentUser.role !== "admin" && currentUser.acessoStats && (
                <button onClick={()=>setAba("stats")} style={{background:"#8e44ad", border:"none", color:"white", cursor:"pointer", marginLeft:"10px", borderRadius:"4px", padding:"5px 10px"}}>📊 Stats</button>
            )}
        </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{flex: 1, padding: "20px", overflowY: "auto"}}>
            {pedidoAtivo ? (
                <div style={{maxWidth: "800px", margin: "0 auto"}}>
                    <div style={{marginBottom: "20px", textAlign: "right"}}>
                        <span style={{background:"#2c3e50", color:"white", padding:"5px 10px", borderRadius:"15px", fontSize:"12px"}}>{mapearStatus(pedidoAtivo.status).toUpperCase()}</span>
                    </div>
                    <Details 
                        ativo={pedidoAtivo} 
            ...
            ) : <div style={{textAlign:"center", color:"#ccc", marginTop:"50px"}}>Selecione um pedido</div>}
        </div>
      </div>
    </div>
  );
